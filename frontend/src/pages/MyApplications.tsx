...
// Visual state machine: Applied → Shortlisted → Selected (rejected/withdrawn are terminal)
      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s.k} className="flex items-center">
            ...
            {i < 2 && (
              <span className="h-[1.5px] min-w-[22px] mx-2.5 flex-shrink-0"
      ...
      {history.map((h: any, i: number) => (
        <div key={i} className="flex gap-3 pb-3">
          ...
          <div>
            ...
            {(h as any).note && <div className="text-[12.5px] text-ink-sub mt-0.5 leading-snug">{(h as any).note}</div>}
          </div>
        </div>
      ))}
    </div>
              ...
              {filtered.map((a) => {
                ...
                return (
                  <div key={a.id} className="panel p-[18px]">
                    ...
                    <div className="flex gap-4 items-start">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1.5">
                          <span className="badge">{a.opportunity_title?.split(" ").slice(-1)[0] === "Trial" ? "Trial" : "Opportunity"}</span>
                        ...
                        <Link to={`/opportunities/${a.opportunity_id}`} className="font-disp text-xl leading-tight hover:text-brand-500 transition">
                          {a.opportunity_title}
                        </Link>
                      </div>
                    ...
                    {/* Stage-specific notes — zip 2 pattern */}
                    {a.status === "shortlisted" && (
                      <div className="mt-3 px-3 py-3 rounded text-[13px] text-blue-900 leading-snug"
                        style={{ background: "#E2EAF8" }}>
                    ...
                    {a.status === "selected" && (
                      <div className="mt-3 px-3 py-3 rounded text-[13px] text-emerald-900 leading-snug"
                        style={{ background: "#E2F0E8" }}>
                    ...
                    {a.status === "rejected" && (lastHistory as any)?.note && (
                      <div className="mt-3 px-3 py-3 rounded text-[13px] text-red-900 leading-snug"
                        style={{ background: "#F8E3E0" }}>
                    ...
                    {/* Expandable history timeline — zip 2 pattern */}
                    {isExpanded && a.history?.length > 0 && (
