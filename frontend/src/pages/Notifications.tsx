      ...
      {list.isLoading ? (
        ...
        <div className="panel divide-y divide-hairsoft">
          {list.data.map((n) => {
            ...
            return (
              <div key={n.id} className={`flex gap-4 px-5 py-4 transition hover:bg-fill/50 ${!n.read ? "bg-fill" : ""}`}>
                <span className="w-9 h-9 rounded-full border border-hair flex items-center justify-center text-[13px] flex-shrink-0 bg-panel"
                  style={{ color }}>
                  {icon}
                </span>
                <div className="flex-1 min-w-0">
                  ...
                  {n.body && <div className="text-sm text-ink-sub mt-0.5 leading-snug">{n.body}</div>}
