...
function MatchRow({ match }: { match: any }) {
  ...
  return (
    <Link to={`/scoring/matches/${match.id}`} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
...
function StandingsTable({ tournamentId }: { tournamentId: string }) {
  const { data } = useQuery({
    ...
    queryFn: () => api.get(`/scoring/tournaments/${tournamentId}/standings`).then(r => r.data)
  });
...
export default function TournamentDetail() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/scoring/tournaments/${id}`).then(r => r.data.tournament)
  });
      ...
      <div className="card overflow-hidden">
        ...
        <div className="p-6">
          <div className="flex items-start gap-4">
            ...
            {canManage && (
              <Link to={`/scoring/tournaments/${id}/edit`} className="btn-secondary shrink-0">
