              ...
              <tr key={e.id} className="border-b border-gray-100">
                <td className="py-2.5 px-3">
                  <div className="flex items-center gap-1">
                    <Link to={`/scoring/players/${e.player.id}`} className="font-medium hover:text-emerald-600 transition-colors">
                      {e.player.name}
                    </Link>
...
export default function MatchDetail() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/scoring/matches/${matchId}`).then(r => r.data.match),
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <Link to={`/scoring/tournaments/${data.tournament_id}`} className="text-sm text-emerald-600 hover:underline flex items-center gap-1">
        ...
        {canManage && data.status === "live" && (
          <Link to={`/scoring/matches/${matchId}/score`} className="btn-primary ml-auto">
            <Radio className="w-4 h-4" /> Score Match
          </Link>
        )}
        {canManage && data.status !== "live" && data.status !== "completed" && (
          <Link to={`/scoring/matches/${matchId}/score`} className="btn-secondary ml-auto">
            <Edit2 className="w-4 h-4" /> Manage
          </Link>
        )}
        {canManage && (
          <Link to={`/scoring/matches/${matchId}/config`} className="btn-secondary text-sm">
            Config
          </Link>
        )}
      </div>
      ...
      {isCricket && data.innings.length > 0 ? (
        ...
        <div className="card p-8 text-center text-gray-400">
          ...
          {canManage && data.status !== "completed" && (
            <div className="mt-3">
              <Link to={`/scoring/matches/${matchId}/score`} className="btn-primary mx-auto">Start Scoring</Link>
            </div>
          )}
        </div>
      )}
