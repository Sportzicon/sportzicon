...
// Lives behind /scoring/matches/:matchId/config and is linked from MatchDetail when
...
export default function MatchConfig() {
  ...
  const { data: tournament } = useQuery({
    ...
    queryFn: () => api.get(`/scoring/tournaments/${match.tournament_id}`).then(r => r.data.tournament),
  ...
  const save = useMutation({
    mutationFn: () => api.put(`/scoring/matches/${matchId}/config`, cfg),
  ...
  return (
    <div className="space-y-5 max-w-3xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Link to={`/scoring/matches/${matchId}`} className="text-sm text-gray-500 hover:text-gray-700 inline-flex items-center gap-1">
      ...
      <div className="flex gap-3">
        ...
        <button onClick={() => navigate(`/scoring/matches/${matchId}`)} className="btn-secondary">Cancel</button>
      </div>
    </div>
  );
}
