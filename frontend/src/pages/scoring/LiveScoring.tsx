...
export default function LiveScoring() {
  ...
  const { data: ballsData } = useQuery({
    ...
    queryFn: () => api.get(`/scoring/innings/${activeInningsId}/balls`).then(r => r.data.balls),
    enabled: Boolean(activeInningsId),
    refetchInterval: 5_000
  });

  const addBallMutation = useMutation({
    mutationFn: (input: any) => api.post(`/scoring/innings/${activeInningsId}/balls`, input),
    onSuccess: () => {
  ...
  const undoMutation = useMutation({
    mutationFn: () => api.post(`/scoring/innings/${activeInningsId}/balls/undo`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["match", matchId] });
      qc.invalidateQueries({ queryKey: ["balls", activeInningsId] });
  ...
  const createInningsMutation = useMutation({
    mutationFn: (input: any) => api.post(`/scoring/matches/${matchId}/innings`, input),
    onSuccess: (r) => {
      qc.invalidateQueries({ queryKey: ["match", matchId] });
  ...
  const completeMatchMutation = useMutation({
    mutationFn: (input: any) => api.put(`/scoring/matches/${matchId}`, { status: "completed", ...input }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["match", matchId] });
      navigate(`/scoring/matches/${matchId}`);
    }
  });
    ...
    <div className="space-y-4 max-w-3xl mx-auto">
      ...
      <div className="card p-4 flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          {activeInnings && (
            <Link to={`/scoring/innings/${activeInnings.id}/analytics`} className="btn-secondary text-sm inline-flex items-center gap-1">
