...
export default function TournamentList() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/tournaments?${new URLSearchParams({ ...(sport && { sport }), ...(status && { status }) })}`).then(r => r.data)
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((t: any) => (
            <Link key={t.id} to={`/tournaments/${t.id}`} className="card p-5 hover:shadow-md transition-shadow group">
